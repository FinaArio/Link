...
export const fetchPrice = () => async dispatch =>{
    ...
    try {
        request.get("https://api.coinmarketcap.com/v1/ticker/?limit=42/?convert=ETH").then(
            res => dispatch({
...
export const fetchNewsById = (id) => async dispatch => {
    dispatch({ type: FETCH_NEWS_BY_ID_START })

    try {
        const newe = await fetchNewsByIdApi(id)
        dispatch({
